...
def Solve(grid:np.ndarray, CAPACITY:int):
  ...
  def npToTuple(nmatrix: np.matrix) -> tuple:
      """
      Converts a NumPy matrix (np.matrix) into a tuple of tuples.

      Args:
          nmatrix: A 2D NumPy matrix (np.matrix).

      Returns:
          A tuple of tuples representing the matrix data.

      NOTE: This function was generated by Gemini, google AI
      """
      # Convert the input np.matrix to a standard np.ndarray first.
      # This ensures that iteration and .tolist() behave as expected for arrays.
      ndarray_representation = np.asarray(nmatrix)

      # Convert the ndarray to a list of lists.
      # For a 2D ndarray, .tolist() directly returns a list of lists.
      list_of_lists = ndarray_representation.tolist()

      # Finally, convert each inner list to a tuple, and the outer list to a tuple.
      return tuple(tuple(row) for row in list_of_lists)
  ...
  heap = []
  # tuple of current box, current weight, remaining boxes
  explored_state = set()
  ...
  while heap: 
    ...
    if (curr_node, curr_weight, npToTuple(curr_grid)) in explored_state:
      continue
    ...
    for next_node, reset in nodes_to_add:
      ...
      if np.all(next_grid==0):
        ...
        if (next_node, next_weight, npToTuple(next_grid)) not in explored_state:
          heapq.heappush(heap, (next_cost, next_node, next_weight, next_encoded_allocation, next_grid))
    explored_state.add((curr_node, curr_weight, npToTuple(curr_grid)))
